...
const PersonalAwoDashboard: React.FC<PersonalAwoDashboardProps> = ({
  ...
  const { data: personalAwo, isLoading } = useQuery<PersonalAwo | null>({
  ...
  if (isLoading) {
    return (
      <div className="min-h-[400px] flex items-center justify-center p-6 bg-background rounded-[2.5rem]">
  ...
  if (!personalAwo) {
    return (
      <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 font-sans">
        {/* Empty State / Welcome */}
        <div className="bg-card rounded-[2.5rem] p-8 md:p-12 text-center border border-border shadow-xl overflow-hidden relative">
          <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-highlight/20 via-highlight to-highlight/20"></div>
          <div className="w-24 h-24 bg-muted rounded-full flex items-center justify-center mx-auto mb-6 text-highlight border border-border shadow-sm">
            <UserPlus size={40} />
          </div>

          <h1 className="text-3xl md:text-5xl font-bold brand-font text-foreground mb-4 tracking-tight">
            Begin Your Journey
          </h1>
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 font-sans">
      ...
      <div className="bg-card rounded-[2.5rem] p-8 md:p-10 shadow-xl border border-border relative overflow-hidden group">
        ...
        <div className="absolute bottom-0 left-0 w-64 h-64 bg-muted/30 rounded-full blur-2xl translate-y-1/2 -translate-x-1/2 pointer-events-none"></div>
        <div className="absolute top-6 right-8 text-[200px] leading-none text-muted/20 font-bold -z-10 select-none opacity-50 brand-font">
          Ifá
        </div>

        <div className="relative z-10">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-muted border border-border rounded-full text-xs font-bold text-highlight uppercase tracking-widest mb-6">
            <span className="w-2 h-2 rounded-full bg-highlight animate-pulse"></span>
            <span>Daily Energy: Eji Ogbe</span>
          </div>

          <div className="max-w-3xl">
            <h1 className="text-4xl md:text-6xl font-bold brand-font text-foreground mb-4 tracking-tight">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 space-y-4">
          <div className="flex items-center justify-between px-2">
            <h2 className="text-2xl font-bold brand-font text-foreground">Your Spiritual Guide</h2>
            {onChangeAwo && (
              <button
                onClick={onChangeAwo}
                className="text-xs font-bold text-muted-foreground hover:text-highlight uppercase tracking-wider transition-colors px-3 py-1 rounded-lg hover:bg-muted"
              >
                Change Guide
              </button>
            )}
          ...
          <div className="bg-card rounded-[2rem] p-6 md:p-8 shadow-lg border border-border relative overflow-hidden group hover:shadow-xl transition-all duration-300">
            {/* Card Shine */}
            <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-white/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none transform -translate-x-full group-hover:translate-x-full"></div>

            <div className="flex flex-col sm:flex-row gap-6 items-start relative z-10">
              {/* Avatar */}
              <div className="relative shrink-0">
                <div className="w-24 h-24 md:w-32 md:h-32 rounded-[2rem] bg-muted overflow-hidden shadow-inner ring-4 ring-card">
                  {babalawo.avatar ? (
                    <img src={babalawo.avatar} alt={babalawo.name} className="w-full h-full object-cover" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-4xl font-bold text-muted-foreground brand-font">
              ...
              <div className="flex-1 space-y-3 pt-2">
                <div>
                  <h3 className="text-2xl font-bold text-foreground leading-tight">{babalawo.name}</h3>
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  <div className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-muted/50 rounded-lg text-xs font-bold text-foreground border border-border">
        ...
        <div className="lg:col-span-5 space-y-4">
          <h2 className="text-2xl font-bold brand-font text-foreground px-2">Actions</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4">

            <button
              onClick={onRequestConsultation}
              className="bg-card p-5 rounded-3xl border border-border shadow-sm hover:shadow-md hover:border-highlight/30 hover:-translate-y-1 transition-all text-left group flex items-start gap-4"
            >
