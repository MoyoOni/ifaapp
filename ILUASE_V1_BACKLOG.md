...
- **VND-026** Vendor Performance Tiers — tiers/badges/sort-tiebreaker are real and backed by a real nightly cron (`vendor-performance-tier.service.ts`). 🔵 Commission deduction itself is now fixed (see 🔴 Critical, resolved July 28, 2026), so the mechanism to hook a discount into exists — but 🔵 checked July 28, 2026: **no discount percentages for the 4 tiers (NEW_VENDOR/ESTABLISHED/TRUSTED_VENDOR/SACRED_ARTISAN) are specified anywhere** — not in this doc, not in `vendor-performance-tier-panel.tsx`'s copy, not in any source doc. This is a real business decision (exact %s directly change vendor payouts and platform revenue), not an engineering detail to invent unilaterally — needs product/business input on the actual discount schedule before implementing. Engineering-wise it's now trivial: read `vendor.performanceTier` in `releaseEscrow()`, apply a per-tier multiplier to `commissionPct`.
