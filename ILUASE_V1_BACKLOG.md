...
- **VND-026** Vendor Performance Tiers — tiers/badges/sort-tiebreaker are real and backed by a real nightly cron (`vendor-performance-tier.service.ts`). 🔵 Commission deduction itself is now fixed (see 🔴 Critical, resolved July 28, 2026), but the tier *benefit* (reduced commission, waived fees) still isn't wired to it — `releaseEscrow()`'s new commission logic reads only `PlatformSettings.marketplaceCommissionPct`, not the vendor's tier. Now a small, well-scoped follow-up rather than blocked on a non-existent base.
