...
describe('BabalawoClientService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [BabalawoClientService, { provide: PrismaService, useValue: mockPrismaService }],
    }).compile();
  ...
  describe('getClients', () => {
    it('should return babalawo clients', async () => {
      const mockClients = [{ id: 'rel-1', clientId: 'client-1', status: 'ACTIVE' }];
    ...
    it('should throw ForbiddenException when accessing other babalawo clients', async () => {
      await expect(service.getClients('other-babalawo', mockCurrentUser)).rejects.toThrow(
        ForbiddenException
      );
    });
  });
