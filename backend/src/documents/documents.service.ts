...
export class DocumentsService {
   ...
   */
  async getSignedUrl(documentId: string, currentUser: CurrentUserPayload) {
    ...
    // Access control (EMG-04) — must check the authenticated identity from the
    // JWT, not a client-supplied route param. This previously compared the
    // document's owner/sharer against an attacker-controlled `:userId` path
    // segment instead of `currentUser.id`, so setting that segment to the
    // victim's ID made the check pass regardless of who was actually logged in.
    if (document.uploadedBy !== currentUser.id && document.sharedWith !== currentUser.id) {
   ...
   */
  async deleteDocument(documentId: string, currentUser: CurrentUserPayload) {
    const document = await this.prisma.document.findUnique({
      where: { id: documentId },
    });
    ...
    // EMG-04: authorize against the authenticated identity, not the
    // client-supplied `:userId` route param — see getSignedUrl for the same fix.
