...
export class SecurityHardeningService {
  ...
  async validateSecurityConfiguration(): Promise<boolean> {
    ...
    if (nodeEnv === 'production') {
      ...
      if (!paystackSecretKey) {
        errors.push('PAYSTACK_SECRET_KEY must be set — required for webhook signature verification');
      }

      const flutterwaveSecretHash = this.configService.get<string>('FLUTTERWAVE_SECRET_HASH');
      if (!flutterwaveSecretHash) {
        errors.push('FLUTTERWAVE_SECRET_HASH must be set — required for webhook signature verification');
      }

      const paystackWebhookSecret = this.configService.get<string>('PAYSTACK_WEBHOOK_SECRET');
      if (!paystackWebhookSecret) {
        errors.push('PAYSTACK_WEBHOOK_SECRET must be set — required for subscription webhook signature verification');
      }
    }
  ...
  async verifyRateLimitingConfiguration(): Promise<void> {
    ...
    if (ttl !== 60000 || limit !== 100) {
      this.logger.warn(`Rate limiting configuration differs from recommended: TTL=${ttl}ms, LIMIT=${limit}`);
    } else {
